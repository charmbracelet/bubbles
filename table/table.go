...
func DefaultStyles() Styles {
	return Styles{
		...
		SelectedCell: lipgloss.NewStyle().Bold(true).Foreground(lipgloss.Color("212")).Underline(true),
...
// CursorCol returns the index of the selected column.
...
// SetCursorCol sets the column cursor position in the table.
func (m *Model) SetCursorCol(n int) {
	m.cursorCol = clamp(n, 0, len(m.cols)-1)
	m.UpdateViewport()
}
...
// MoveLeft moves the column selection left by any number of columns.
// It can not go before the first column.
func (m *Model) MoveLeft(n int) {
	m.cursorCol = clamp(m.cursorCol-n, 0, len(m.cols)-1)
	m.UpdateViewport()
}

// MoveRight moves the column selection right by any number of columns.
// It can not go beyond the last column.
func (m *Model) MoveRight(n int) {
	m.cursorCol = clamp(m.cursorCol+n, 0, len(m.cols)-1)
	m.UpdateViewport()
}

// SelectedCell returns the value of the currently selected cell (intersection
// of selected row and column). Returns an empty string if the selection is
// out of bounds.
func (m Model) SelectedCell() string {
	row := m.SelectedRow()
	if row == nil || m.cursorCol < 0 || m.cursorCol >= len(row) {
		return ""
	}
...
func (m *Model) renderRow(r int) string {
	s := make([]string, 0, len(m.cols))
	cellSelectionEnabled := m.KeyMap.LineLeft.Enabled() || m.KeyMap.LineRight.Enabled()
	...
	for i, value := range m.rows[r] {
		...
		if r == m.cursor && cellSelectionEnabled && i == m.cursorCol {
			cellStyle = m.styles.SelectedCell
		}
