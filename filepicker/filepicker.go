...
func (m *Model) evalSymlinks(name string) (string, error) {
	...
	// Since go 1.25
	if sym, ok := m.FS.(fs.ReadLinkFS); ok {
		return sym.ReadLink(path.Join(m.CurrentDirectory, name))
	}

	// Fallback because io/fs.ReadLink just fails if ReadLinkFS is not implemented.
