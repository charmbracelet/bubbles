...
type StyleFunc func(children Items, i int) lipgloss.Style
...
type Styles struct {
	...
	selectedItemFunc      StyleFunc
	SelectedItemStyle     lipgloss.Style
	SelectedItemStyleFunc StyleFunc
	itemFunc              StyleFunc
...
func DefaultStyles() (s Styles) {
	s.SelectedNode = lipgloss.NewStyle().
		...
		Bold(true)
	s.itemFunc = func(items Items, index int) lipgloss.Style {
		item := items.At(index)
		if item.IsSelected() {
...
func (t *Item) IsSelected() bool {
	return t.yOffset == t.opts.treeYOffset
}

// Depth returns the depth of the node in the tree.
func (t *Item) Depth() int {
	return t.depth
}

// Size returns the number of nodes in the tree.
// Note that if a child isn't open, its size is 1
func (t *Item) Size() int {
	return t.size
}

// YOffset returns the vertical offset of the Item
func (t *Item) YOffset() int {
	return t.yOffset
}
...
// TODO
const IndenterWidth = 4

func (t *Item) contentWidth() int {
	c := max(0, (t.depth-1)*IndenterWidth+EnumeratorWidth) + lipgloss.Width(t.tree.Value())
	if t.isRoot {
		c += lipgloss.Width(t.opts.openCharacter + " ")
	}
	return c
}
...
func (t *Item) Value() string {
	s := lipgloss.NewStyle()
	// padding := strings.Repeat("·", max(0, t.opts.treeWidth-t.contentWidth()))
	padding := ""
	if t.isRoot {
		if t.open {
			return s.Render(t.opts.openCharacter + " " + t.tree.Value() + padding)
		}
		return s.Render(t.opts.closedCharacter + " " + t.tree.Value() + padding)
	}
	return s.Render(t.tree.Value() + padding)
}
...
// Children returns the children of an item.
func (t Items) At(index int) *Item {
	return t[index]
}

// Children returns the children of an item.
func (t Items) Length() int {
	return len(t)
}

// ItemStyle sets a static style for all items.
func (t *Item) ItemStyle(s lipgloss.Style) *Item {
	t.tree.ItemStyle(s)
	return t
}
...
func (t *Item) Enumerator(enumerator ltree.Enumerator) *Item {
...
func (t *Item) EnumeratorStyle(style lipgloss.Style) *Item {
...
//	│   └── Baz
//	└── Qux
func (t *Item) Child(children ...any) *Item {
...
func New(t *Item, width, height int) Model {
	m := Model{
...
func (m *Model) SetStyles(styles Styles) {
	if styles.ItemStyleFunc != nil {
		styles.itemFunc = styles.ItemStyleFunc
	} else {
		styles.itemFunc = func(_ Items, _ int) lipgloss.Style {
			return styles.ItemStyle
		}
	}
	if styles.SelectedItemStyleFunc != nil {
		styles.selectedItemFunc = styles.SelectedItemStyleFunc
	} else {
		styles.selectedItemFunc = func(_ Items, _ int) lipgloss.Style {
...
// Note that if a child isn't open, its size is 1
func setDepths(t *Item, depth int) {
	t.depth = depth
	children := t.tree.Children()
	for i := 0; i < children.Length(); i++ {
		child := children.At(i)
		setDepths(child.(*Item), depth+1)
	}
}
...
func (m *Model) selectedNodeStyle() StyleFunc {
	return func(children Items, i int) lipgloss.Style {
		child := children.At(i)
		if child.yOffset == m.yOffset {
			return m.styles.selectedItemFunc(children, i)
		}

		return m.styles.itemFunc(children, i)
	}
}
...
func printDebugInfo(t *Item) string {
	debug := fmt.Sprintf("size=%2d y=%2d depth=%2d padding=%2d", t.size, t.yOffset, t.depth, t.contentWidth())
	children := t.Children()
	for i := 0; i < children.Length(); i++ {
		child := children.At(i)
		if child, ok := child.(*Item); ok {
